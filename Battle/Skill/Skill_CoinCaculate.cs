...
public class Skill_CoinCaculate : MonoBehaviour
{
    ...
    public (bool coin_bool,int coin_Value_result, int Coin_Value_Calculated) Coin_Value_Calculate()
    {
        
        bool coin_bool = true;//코인 앞뒤 계산 (캐릭터 에서 계산)
        //코인값 얻기 
        int coin_Value_result = skill_Main.Coin_GetValue(skill_Coin_Value_Cur,coin_bool);
        //해당값 바탕으로 type에따라 값 적용시키기
        int Coin_Value_Calculated = skill_Main.Coin_Calculate(ref skill_Value_Cur,coin_Value_result,skill_Coin_CalculateType);
    ...
    public void Skill_Calculate()
    {
        ...
        {
            var logData = Coin_Value_Calculate();
            Debug.Log($"코인 : {logData.coin_bool}|코인 결과값 : {logData.coin_Value_result}|스킬 위력 : {logData.Coin_Value_Calculated}");
        }
